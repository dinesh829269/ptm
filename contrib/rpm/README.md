...
The RPM spec file provided here is for bitphantom-Core 0.12.0 and builds on CentOS
...
necessary, the packager should bring the problem to the attention of the bitphantom
...
needed when 0.12.1 is released, a proper fix is already in the bitphantom git
master branch.
...
idea to build bitphantom Core against a static version of that library compiled
...
The `bitphantom.svg` file is from the source listed as `Source100`. It is used as
...
matters for the `bitphantom-server` package. At this point, most RPM based
...
The `bitphantom-server` package should have SELinux support. How to properly do
...
bitphantom-Core against OpenSSL or use a patched version of Boost in the build
system.
...
[Ringing Liberty](https://www.ringingliberty.com/bitphantom/). He has been
packaging bitphantom for Fedora at least since 2012.
...
4. I renamed the `bitphantom` package that contains the Qt GUI to `bitphantom-core` as
that appears to be how the general population refers to it, in contrast to
`bitphantom-xt` or `bitphantom-classic`. I wanted to make sure the general population

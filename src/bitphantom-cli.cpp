...
static void SetupCliArgs()
{
    ...
    gArgs.AddArg("-rpcwallet=<walletname>", "Send RPC for non-default wallet on RPC server (needs to exactly match corresponding -wallet option passed to bitphantomd)", false, OptionsCategory::OPTIONS);
...
static int AppInitRPC(int argc, char* argv[])
{
    ...
    if (argc < 2 || HelpRequested(gArgs) || gArgs.IsArgSet("-version")) {
        ...
        if (!gArgs.IsArgSet("-version")) {
            strUsage += "\nUsage:\n"
                  "  bitphantom-cli [options] <command> [params]  " + strprintf("Send command to %s", PACKAGE_NAME) + "\n" +
                  "  bitphantom-cli [options] -named <command> [name=value] ... " + strprintf("Send command to %s (with named arguments)", PACKAGE_NAME) + "\n" +
                  "  bitphantom-cli [options] help                List commands\n" +
                  "  bitphantom-cli [options] help <command>      Get help for a command\n";
